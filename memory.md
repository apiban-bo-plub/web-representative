...
- Redesigned the `/video-test` page to support a true parallax reveal sequence. The video is positioned in a fixed background layer, while the scroll layer starts with a transparent `100vh` spacer (passing pointer-events through to the video play/mute controls initially). As the user scrolls, the fixed background video scales and fades out, while the actual homepage Hero section slides up from below the fold on top of the video, creating a perfect parallax entrance.
- Bound the homepage Hero container's opacity directly to the scroll progress (`style={{ opacity: progress }}`). This creates a dynamic cross-fade transition where the Hero section fades in from `0` to `100%` as the background video fades out from `100%` to `0` (and vice-versa when scrolling back up).
...
## Active Tasks
- None. All current user requests (i18n dropdown, mobile drawer tweaks, shop page cart removal, photo renaming/mapping, video embed crop overlays, scroll-driven parallax reveals, Hero opacity cross-fading, and scroll-responsive navbar animations) are fully complete and validated.
